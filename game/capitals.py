...
def check_answer(message, bot: telebot, user: dict, city: str) -> None:
    if 'timer' not in user:
        ...
        if user['hits'] == NUM_COUNTRIES:
            bot.send_message(message.chat.id, ("&lt---------- " + '<b><i>ИДЕАЛЬНЫЙ РАУНД</i></b>' + " ----------&gt"),
        ...
        user['timer'].cancel()
        bot.send_chat_action(message.chat.id, "typing")
        bot.send_message(message.chat.id, f"<b><i>НЕПРАВИЛЬНО</i></b>, правильный ответ: <b><i>{city}</i></b>",
        ...
        time.sleep(1)
        time.sleep(1)
        bot.send_message(message.chat.id, "Используйте /capitals, чтобы сыграть снова")
...
def next_question(message, bot: telebot, user: dict) -> None:
    ...
    user['timer'].start()
    answer = bot.send_message(message.chat.id, (f"{countries[country]['country']}" + " " +
                                                f"{countries[country]['flag'].encode().decode('unicode_escape')}"),
                              reply_markup=markup)
